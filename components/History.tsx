...
const LiquidChart = ({ efficiency, balance, isPositiveBalance }: { efficiency: number, balance: number, isPositiveBalance: boolean }) => {
    ...
    const theme = useMemo(() => {
        if (efficiency >= 100) return { primary: '#14b8a6', dark: '#0f766e', glow: 'rgba(20, 184, 166, 0.4)', glowDim: 'rgba(20, 184, 166, 0.1)', icon: '🚀' };
        if (efficiency >= 80) return { primary: '#3b82f6', dark: '#1d4ed8', glow: 'rgba(59, 130, 246, 0.4)', glowDim: 'rgba(59, 130, 246, 0.1)', icon: '⚡' };
        return { primary: '#f43f5e', dark: '#be123c', glow: 'rgba(244, 63, 94, 0.4)', glowDim: 'rgba(244, 63, 94, 0.1)', icon: '⚠️' };
    }, [efficiency]);
...
const TeamView: React.FC<TeamViewProps> = ({ user, records, shiftConfig }) => {
  ...
  if (view === 'JOIN') {
      return (
        <div className="pt-8 px-6 pb-32 animate-fade-in min-h-screen relative bg-slate-50 transform-gpu">
  ...
  if (view === 'CREATE') {
    return (
        <div className="pt-8 px-6 pb-32 animate-fade-in min-h-screen relative overflow-hidden transform-gpu">
             <div className="absolute top-[-20%] right-[-20%] w-[500px] h-[500px] bg-blue-500/10 rounded-full blur-[40px] pointer-events-none transform-gpu" />
             ...
             <div className="bg-white/60 backdrop-blur-xl rounded-[40px] p-8 shadow-2xl shadow-blue-900/10 border border-white relative overflow-hidden transform-gpu">
...
const InviteModal = ({ code, onClose }: { code: string, onClose: () => void }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 transform-gpu">
...
const MemberActionSheet = ({ isOpen, member, onClose, onRemove }: { isOpen: boolean, member: GroupMember | null, onClose: () => void, onRemove: () => void }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[2000] flex items-end justify-center sm:items-center p-0 sm:p-4 pointer-events-none transform-gpu">
             ...
             <div className="bg-white w-full max-w-sm rounded-t-[32px] sm:rounded-[32px] p-6 pb-8 z-10 shadow-2xl transform transition-transform animate-slide-up mx-auto mb-0 sm:mb-auto pointer-events-auto transform-gpu">
...
const MemberDetailModal = ({ isOpen, member, onClose }: { isOpen: boolean, member: GroupMember | null, onClose: () => void }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[1000] flex items-end justify-center sm:items-center transform-gpu">
