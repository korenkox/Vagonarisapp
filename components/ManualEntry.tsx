...
const ManualEntry: React.FC<ManualEntryProps> = ({ onSave, onDelete, user, records = [], shiftConfig }) => {
  ...
  const getTheme = (pct: number) => {
      ...
      if (pct >= 95) {
          return {
              ...
              glow: 'none', // No glow for calm state
              noEffects: true // Disables animations
          };
      }
      // 3. RED (< 95%) - Default
      return {
  ...
  // Cap visual stroke at 100% (full circle), logic handles value display
  const strokeDashoffset = circumference - (Math.min(animatedPercentage, 100) / 100) * circumference;
  ...
  return (
    <div className="pt-2 px-5 pb-24 animate-fade-in font-sans relative compact-mode-padding">
      ...
      {/* CSS Styles from source code adapted for React */}
      <style>{`
        /* Animations from source */
        @keyframes pulseGlow { 
        ...
        /* Classes for manual implementation */
        .chart-glow {
            animation: pulseGlow 2s infinite ease-in-out;
        }
        .svg-animate {
            animation: pulseWholeGraph 2s infinite ease-in-out;
        }
        .dashed-outer {
        ...
        .status-icon-anim {
            animation: popIn 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) 0.5s forwards, float 3s ease-in-out 1.1s infinite;
            transform: scale(0); /* Start hidden for popIn */
        }
        .stat-shimmer::after {
            content: '';
            position: absolute;
      ...
      <button 
        ...
        className={`w-full relative py-4 rounded-[20px] overflow-hidden group transition-all duration-500 shadow-[0_15px_30px_-10px_rgba(15,23,42,0.3)] mb-6 ${isAnalyzing ? 'opacity-90' : 'hover:scale-[1.01]'}`}
      >
      ...
      <div className={`w-full rounded-[40px] p-[22px] shadow-[0_30px_60px_rgba(0,0,0,0.12)] relative overflow-hidden bg-white mb-8 transition-all duration-500 ${theme.noEffects ? 'no-effects' : ''}`}>
          ...
          <div className="flex justify-between items-start mb-[20px]">
              <div className="flex gap-[15px] items-center">
                  <div className="w-[42px] h-[42px] rounded-[14px] flex items-center justify-center font-[800] text-[1.2rem] shadow-[0_4px_12px_rgba(0,0,0,0.1)] transition-colors duration-500"
                       style={{ backgroundColor: theme.lightBg, color: theme.primaryColor }}>
                      {theme.headerIcon}
                  </div>
                  <div className="text-[1.15rem] font-[800] text-[#2D3748] leading-[1.2] uppercase tracking-tighter">
                      MESAČNÝ<br />VÝKON
                  </div>
              </div>
              <div className="px-[14px] py-[8px] rounded-[20px] text-[0.75rem] font-[800] uppercase shadow-[0_2px_8px_rgba(0,0,0,0.03)] border transition-all duration-500"
                   style={{ borderColor: theme.lightBg, color: theme.primaryColor, backgroundColor: 'rgba(255,255,255,0.5)' }}>
                  {dateBadgeText}
              </div>
          </div>

          {/* Top Row: Chart + Worked */}
          <div className="flex flex-row items-center justify-between gap-2">
              ...
              <div className="relative w-[130px] h-[130px] flex-shrink-0 flex items-center justify-center -ml-2">
                  ...
                  <svg width="130" height="130" viewBox="0 0 160 160" className="relative z-10 overflow-visible svg-animate">
                      <defs>
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 flex flex-col items-center justify-center text-pulse">
                      <div className="text-[1.6rem] font-[800] leading-none bg-clip-text text-transparent transition-all duration-500"
                      ...
                      <div className="text-[0.5rem] font-[700] text-[#A0AEC0] uppercase mt-[4px] tracking-[0.05em] label-appear">
                          Účinnosť
                      </div>
                  </div>

                  <div className="absolute top-[22px] right-[18px] w-[18px] h-[18px] bg-white rounded-full flex items-center justify-center shadow-[0_4px_8px_rgba(0,0,0,0.15)] z-30 status-icon-anim"
                       style={{ color: theme.primaryColor }}>
                       {React.cloneElement(theme.statusIcon as React.ReactElement<any>, { size: 10 })}
                  </div>
              </div>
              
              {/* Odpracované Box - Side by Side with Chart */}
              <div className="stat-shimmer flex-1 min-w-0 p-[15px_18px] rounded-[20px] relative overflow-hidden transition-colors duration-500 flex flex-col justify-center h-[110px]"
                    style={{ backgroundColor: theme.lightBg }}>
                    <div className="text-[0.65rem] font-[700] uppercase mb-[6px] flex justify-between items-center transition-colors duration-500"
                        style={{ color: theme.primaryColor }}>
                        Odpracované
                        <Clock size={14} />
                    </div>
                    <div className="text-[1.8rem] font-[800] inline-block transition-colors duration-500 leading-tight"
                        style={{ color: theme.gradientEnd }}>
                        {animatedWorked.toFixed(1)}<span className="text-[1rem] font-[600] ml-[2px]">h</span>
                    </div>
              </div>
          </div>

          {/* Bottom Row: Fond and Norma */}
          <div className="grid grid-cols-2 gap-4 mt-6">
               <div className="flex justify-between items-center py-[6px] border-b border-[#F7FAFC] stat-row-2 group">
                  <div>
                      <h4 className="m-0 text-[0.7rem] text-[#A0AEC0] uppercase font-[700]">Fond</h4>
                      <p className="m-[4px_0_0_0] text-[1.1rem] font-[700] text-[#2D3748]">{monthlyStats.fund}h</p>
                  </div>
              ...
              <div className="flex justify-between items-center py-[6px] border-b border-[#F7FAFC] stat-row-1 group">
                  <div>
                      <h4 className="m-0 text-[0.7rem] text-[#A0AEC0] uppercase font-[700]">Norma</h4>
                      <p className="m-[4px_0_0_0] text-[1.1rem] font-[700] text-[#2D3748]">{monthlyStats.target}h</p>
                  </div>
                  <Briefcase size={18} className="text-[#CBD5E0] transition-colors duration-300 group-hover:text-[#E93B3B]" style={{ color: undefined }} />
              </div>
          </div>

          <div className="mt-[25px] text-center text-[0.85rem] font-[700] opacity-0 footer-appear transition-colors duration-500"
               style={{ color: theme.primaryColor, letterSpacing: '0.02em' }}>
              {theme.footerText}
          </div>
      </div>
      {/* --- END CARD --- */}
      ...
      <div className="flex justify-between items-end mb-3">
         ...
         {currentMonthRecords.length > 2 && (
            <button onClick={() => setIsExpanded(!isExpanded)} className="text-xs font-bold text-blue-600 hover:text-blue-700">
        ...
        {visibleRecords.length > 0 ? visibleRecords.map(record => (
           <button key={record.id} onClick={() => setSelectedRecord(record)} className="w-full bg-white rounded-[20px] p-3 flex items-center gap-3 shadow-lg shadow-blue-900/5 border border-white hover:scale-[1.02] active:scale-95 transition-all text-left">
              ...
              <div className="flex-1">
                 ...
                 <div className="text-[10px] text-gray-400 uppercase tracking-wider">{record.totalWorked} odpracované</div>
              </div>
              {record.balance && (
                <div className={`font-bold text-xs px-2.5 py-1 rounded-full ${record.isPositiveBalance !== false ? 'text-teal-600 bg-teal-50' : 'text-rose-600 bg-rose-50'}`}>
                   {record.balance}
                </div>
              )}
           </button>
...
/* --- SUB COMPONENTS (KEEPING EXISTING ONES) --- */
